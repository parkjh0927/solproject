...
<form class="row g-3 justify-content-center" action='<c:url value="/list.do"/>'>
	<div class="col-auto">
		<select class="form-select" name="criteria">
			...
			<option value="title" <c:out value="${pageDTO.criteria=='title'?'selected':'' }" /> >title
		...
		<c:forEach var="dto" items="${list}">
			<tr>
				...
				<td><c:if test="${dto.reLev!=0 }">
					...
					</c:if> <a href='${dto.bno}' class = 'move'>${dto.title}</a></td>
				<td>${dto.name}</td>
				<td>${dto.regDate}</td>
				<td>${dto.cnt}</td>
			</tr>
...
<div class="row">
	...
	<div class="col-2">
		<select name="amount" aria-label="" id="amount1" class="form-select">
...
<nav aria-label="page navigation">
  <ul class="pagination justify-content-center">
    ...
    <c:forEach begin='${pageDTO.startPage}' end="${pageDTO.endPage}" var="id">
    <li class="page-item ${pageDTO.page==id?'active':'' }"><a class="page-link" href="${id}" id="abc">${id}</a></li>
    </c:forEach>
